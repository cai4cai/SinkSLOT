...
def bench_sinkslot(
    ...
    *,
    ...
    seed: data-generation seed only (x, y, a, b). expected_sliced_plan's own projection
    RNG stays at its independent default seed=0, unaffected by this.
    """
    from sinkslot.solver import (
        expected_sliced_plan, to_csr,
    )
    ...
    try:
        ...
        rows, cols, S = expected_sliced_plan(x, y, a, b, L=slices, seed=0)
...
def bench_sinkslotcuda(
    ...
    * ``to_csr(..., narrow_key=True)`` -- int32 CSC sort key (4.96 -> 2.24 ms).
    ...
    """
    from sinkslot.solver import (
        expected_sliced_plan, to_csr, sparse_sqeuclidean_cost, _ot_1d_coo_batched_cuda,
    )
    from sinkslot.sinkhorn_solvers import sinkslot_alternating_triton
    _set_tf32(allow_tf32)
    ...
    def _setup():
        rows, cols, S = expected_sliced_plan(x, y, a, b, L=slices, seed=0, ot1d=_ot_1d_coo_batched_cuda)
