"""Tests for the pure-torch fallback (`sinkhorn_alternating_torch`, `sinkslot_solve`), #10.
...
def test_run_v5_torch_marginal_and_potential_modes_converge_and_agree():
    ...
    phi_m, psi_m, it_m, conv_m, viol_m = sinkhorn_alternating_torch(
        rows, cols, lam, log_a, log_b, n, m, 20000, _Stop(mode="marginal"))
    assert conv_m and viol_m <= 1e-6

    phi_p, psi_p, it_p, conv_p, viol_p = sinkhorn_alternating_torch(
...
def test_run_v5_torch_potential_linf_mode_converges():
    ...
    phi, psi, it, converged, change = sinkhorn_alternating_torch(
...
def test_run_v5_torch_rejects_unknown_mode():
    ...
    with pytest.raises(ValueError, match="unknown stop.mode"):
        sinkhorn_alternating_torch(rows, cols, lam, log_a, log_b, n, m, 100,
