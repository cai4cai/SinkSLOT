...
def slot_grad(X, Y, a, eps, L, seed, n_iters):
    ...
    X/Y/a expected fp32: sparse_sqeuclidean_cost's Triton kernel accumulates in
    fp32 regardless of input dtype, and this is the CUDA setup path
    (_ot_1d_coo_batched_cuda) + the fused Triton solve (_run_v5), so both need a
    CUDA GPU. `stop=None` already runs a fixed n_iters -- see _run_v5's mode
    dispatch -- so there's no separate "fixed-stop" config to build here.
    """
